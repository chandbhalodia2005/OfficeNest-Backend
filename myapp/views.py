...
from .serializers import EmployeeSerializer ,AttendanceSerializer,TaskSerializer,OverallBonusPredictionSerializer
...
class CompanySignupView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            # Save the company with the password directly, without hashing
            company = serializer.save() 
            company.password = serializer.validated_data['password']
            company.save()
...
def login_user(request):
    ...
    try:
        ...
        if company.password == password: # DIRECT PASSWORD COMPARISON
    ...
    try:
        ...
        if manager.password == password: # DIRECT PASSWORD COMPARISON
    ...
    try:
        ...
        if employee.password == password: # DIRECT PASSWORD COMPARISON
...
def bulk_create_users(request):
    try:
        data_json = request.data.get('users_json')
        if not data_json:
            return Response({"error": "No user data found in the request."}, status=status.HTTP_400_BAD_REQUEST)
            ...
            for user_data in users_data:
                ...
                if role == 'manager':
                    ...
                    new_user_data = {
                        ...
                        'shift_start': user_data.get('shift_start'),
                        'shift_end': user_data.get('shift_end'),
                        'company': company
                    }
                elif role == 'employee':
                    Model = Employee
                    new_user_data = {
                        ...
                        'address': user_data.get('address'),
                        'phone': user_data.get('phone'),
                        'position': user_data.get('designation'),
                        'gender': user_data.get('gender'),
                        'date_of_birth': user_data.get('dob'),
                        'salary': user_data.get('salary'),
                        'shift_start': user_data.get('shift_start'),
                    ...
                    if user_data.get('manager'):
                        manager = Manager.objects.get(pk=user_data['manager'])
                        new_user_data['manager'] = manager
                else:
                    continue
                
                user_instance = Model(**new_user_data)
                user_instance.password = user_data.get('password') # Direct assignment
                
                image_idx = user_data.get('face_image_idx')
                if image_idx is not None:
                    image_key = f'face_image_{image_idx}'
                    if image_key in request.FILES:
                        user_instance.face_image = request.FILES[image_key]
...
from .models import Employee, Company # Ensure you import Manager if needed for direct lookup
# Assuming you have an EmployeeSerializer
...
from .models import Employee, Company # Ensure you import Manager too
from .serializers import EmployeeSerializer # Assuming this exists
# In your views.py
...
# This single view handles GET (retrieve) and PATCH (update) requests for a single employee
class EmployeeDetailUpdateView(generics.RetrieveUpdateAPIView):
    queryset = Employee.objects.all()
    serializer_class = EmployeeSerializer
    parser_classes = [MultiPartParser, FormParser]
    lookup_field = 'employee_id' # Specify which field to use for lookup from the URL

    def patch(self, request, *args, **kwargs):
        # Override patch to ensure the file is handled correctly if it exists
        return self.partial_update(request, *args, **kwargs)

    def update(self, request, *args, **kwargs):
        # This custom method will ensure only the necessary fields are updated
        # It's better to use the default partial_update provided by DRF
        # But if you need custom logic, this is where you'd put it.
        # Your current setup works fine without this override.
    ...
    def post(self, request):
        try:
            ...
            if not isinstance(employees_data, list) or not employees_data:
                return Response({"error": "No employee records found in JSON data or invalid format"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            with transaction.atomic():
                for index, emp_data in enumerate(employees_data):
                    raw_password = emp_data.get('password')
                    ...
                    serializer_data = {
                        "name": emp_data.get("fullName"),
                        "email": emp_data.get("email"),
                        "phone": emp_data.get("phone"),
                        "address": emp_data.get("address"),
                        "position": designation,
                        "gender": gender_char,
                        "date_of_birth": emp_data.get("dob"),
                        "date_joined": emp_data.get("joiningDate"),
                    ...
                    if face_image_file:
                        serializer_data["face_image"] = face_image_file
                    ...
                    if serializer.is_valid():
                        ...
                        except (IntegrityError, Company.DoesNotExist, Employee.DoesNotExist) as e:
                            # Catch specific database errors and add them to the errors list
                            print(f"Database save error for employee {index}: {e}")
                            errors.append({"index": index, "data": emp_data, "errors": str(e)})
                    else:
                        print(f"Serializer validation error for employee {index}: {serializer.errors}")
                        errors.append({"index": index, "data": emp_data, "errors": serializer.errors})

            if errors:
                return Response({
                    "message": "Some employees could not be created", 
                    "errors": errors
                }, status=status.HTTP_400_BAD_REQUEST)
...
class EmployeeTaskListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # This is the correct way to filter through the intermediary model
        return Task.objects.filter(
            Q(taskassignment__employee_id=employee_id) | 
            Q(taskassignment__manager_id=employee_id)
        ).prefetch_related(
            'taskassignment_set__employee', 
            'taskassignment_set__manager'
...
from rest_framework.views import APIView
from django.shortcuts import get_object_or_404
from .models import Task, TaskAssignment, Employee, Manager # Make sure you have these models
from .serializers import TaskSerializer, TaskAssignmentSerializer # Make sure you have these serializers
...
class TaskCompletionView(APIView):
    def patch(self, request, pk, *args, **kwargs):
        ...
        if not employee_id and not manager_id:
            return Response(
                {'error': 'Either employee_id or manager_id is required.'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {'error': 'Task assignment not found for this user.'}, 
                status=status.HTTP_404_NOT_FOUND
            )
...
def get_employee_tasks(request):
    ...
    try:
        ...
        return JsonResponse({'error': str(e)}, status=500)
# Make sure to import all other views and add them below this line
# views.py
from rest_framework import generics, status
from rest_framework.response import Response
from .models import Task, TaskAssignment
...
class TaskListCreateView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            company = Company.objects.get(pk=int(company_id))
            queryset = Task.objects.filter(company=company).order_by("-created_at")
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = request.data or {}
        # Use the correct field names from your models (start_date, end_date)
        title = data.get("title")
        description = data.get("description", "")
        start_date = data.get("start")
        end_date = data.get("end")
        ...
        if not all([title, start_date, end_date, assignees]):
            return Response({"error": "Title, start date, end date, and assignees are required."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            # Create the task first, linking it to the company
            task = Task.objects.create(
                title=title,
                description=description,
                start_date=start_date,
                end_date=end_date,
                company=company  # This is crucial and now non-nullable
            )

            # Find personnel and create TaskAssignment instances
            for name in assignees:
                try:
                    # Try to find an employee
                    assigned_user = Employee.objects.get(name=name, company=company)
                    TaskAssignment.objects.create(task=task, employee=assigned_user)
                except Employee.DoesNotExist:
                    try:
                        # Try to find a manager
                        assigned_user = Manager.objects.get(name=name, company=company)
                        TaskAssignment.objects.create(task=task, manager=assigned_user)
                    except Manager.DoesNotExist:
                        # If the assignee name is not found, log and continue
                        print(f"Warning: Assignee '{name}' not found for company {company_id}.")
                        continue
...
class TaskDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        # If assignees are provided, update the M2M relationships
        if isinstance(assignees, list):
            # Delete old TaskAssignment records
            TaskAssignment.objects.filter(task=task).delete()

            # Create new TaskAssignment records
            for name in assignees:
                try:
                    assigned_user = Employee.objects.get(name=name, company=task.company)
                    TaskAssignment.objects.create(task=task, employee=assigned_user)
                except Employee.DoesNotExist:
                    try:
                        assigned_user = Manager.objects.get(name=name, company=task.company)
                        TaskAssignment.objects.create(task=task, manager=assigned_user)
                    except Manager.DoesNotExist:
                        print(f"Warning: Assignee '{name}' not found during task update.")
                        continue
...
class ChangePasswordView(APIView):
    def post(self, request):
        ...
        if not all([company_id, current_password, new_password]):
        ...
        # Use the set_password method to securely hash and save the new password
...
class ChangePasswordView(APIView):
    """
    An API view for changing an employee's password.
    """
    def post(self, request, *args, **kwargs):
...
# This view fetches all tasks assigned to a specific employee or manager
@require_GET
def get_user_tasks(request):
    ...
    if not user_type or not user_id:
        return JsonResponse({'error': 'user_type and user_id are required'}, status=400)
    
    tasks_data = []
    
    try:
        if user_type == 'employee':
            user_instance = get_object_or_404(Employee, employee_id=user_id)
            assignments = TaskAssignment.objects.filter(employee=user_instance).select_related('task')
            for assignment in assignments:
                task = assignment.task
                tasks_data.append({
                    'id': task.id,
            ...
            user_instance = get_object_or_404(Manager, manager_id=user_id)
            assignments = TaskAssignment.objects.filter(manager=user_instance).select_related('task')
            for assignment in assignments:
                task = assignment.task
                tasks_data.append({
                    'id': task.id,
    ...
    except (Employee.DoesNotExist, Manager.DoesNotExist) as e:
        return JsonResponse({'error': f'{user_type.capitalize()} not found'}, status=404)
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)


# This view updates a task assignment's completion status
from rest_framework import status
from rest_framework.response import Response
from rest_framework.views import APIView
from django.http import JsonResponse
...
class TaskCapacityPredictionView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not employee_data or not isinstance(employee_data, list):
                return Response(
                    {'error': 'Invalid or missing employee data.'}, 
                    status=status.HTTP_400_BAD_REQUEST
                )

            # Define the path to your Python script
            # Adjust this path based on where you saved predict_task_capacity.py
            script_path = os.path.join(os.path.dirname(__file__), 'predict_task_capacity.py')

            # Pass the employee data to the script via a pipe
            ...
            # Send the data as a JSON string to the script's standard input
            input_data = json.dumps({'employee_data': employee_data})
            stdout, stderr = process.communicate(input=input_data)
            
            if process.returncode != 0:
                print(f"Script Error: {stderr}")
                return Response(
                    {'error': 'Prediction script failed.', 'details': stderr}, 
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
...
from rest_framework import status
from django.shortcuts import get_object_or_404
from .models import TaskAssignment, Task, Employee, Manager  # Adjust the import paths
from .serializers import TaskBonusPredictionSerializer
import random
import datetime

# Placeholder ML prediction function
def predict_bonus(task_assignment):
    """
    A placeholder function to predict a bonus amount.
    This simulates an ML model's output.
    A real model would use features like task complexity, completion time,
    user's historical performance, etc.
    """
    completion_multiplier = 1.0
    if task_assignment.is_completed_by_user:
        completion_multiplier = 1.5
    
    task_duration = (task_assignment.task.end_date - task_assignment.task.start_date).days
    base_bonus = random.randint(50, 200)
    
    predicted_bonus = base_bonus + (task_duration * 10 * completion_multiplier) + (random.random() * 50)
    
    return predicted_bonus

class TaskBonusPredictionAPIView(APIView):
    def get(self, request, *args, **kwargs):
        company_id = request.query_params.get('company_id')
        if not company_id:
            return Response(
        ...
        for assignment in task_assignments:
            predicted_bonus = predict_bonus(assignment)
            
            user_name = ""
            user_type = ""
            if assignment.employee:
                user_name = assignment.employee.name
                user_type = "Employee"
        ...
        serializer = TaskBonusPredictionSerializer(predictions, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
...
# Placeholder ML prediction function. 
# This is kept the same as before, as it predicts a bonus for a single task.
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
from collections import defaultdict
...
# Import your models from the correct paths.
# your_project/your_app/views.py
...
from .models import TaskAssignment, Employee, Manager
from .serializers import OverallBonusPredictionSerializer

ml_model = None
try:
    MODEL_PATH = os.path.join(settings.BASE_DIR, 'your_app_name', 'bonus_prediction_model.pkl')
    ml_model = joblib.load(MODEL_PATH)
except FileNotFoundError:
    print("WARNING: ML model file not found. Please train the model and ensure the path is correct.")
except Exception as e:
    print(f"ERROR: Failed to load ML model. Reason: {e}")

# UPDATED: This function now calculates the final bonus amount directly.
def predict_overall_bonus(tasks_completed, is_manager):
    # This is the new logic to guarantee a base bonus for completed tasks
    if tasks_completed > 0:
        base_bonus = 100 # A minimum bonus for completing at least one task
    else:
        return 0.0 # Return 0 if no tasks are completed

    if ml_model is None:
        return base_bonus + (tasks_completed * 20) # Fallback calculation
    
    # Calculate a proxy for 'on_time_percentage' as the ML model requires it
    on_time_percentage = 90 + (tasks_completed / 100) * 5
    ...
    # Use the model to predict the bonus class (1 for bonus, 0 for no bonus)
    prediction_class = ml_model.predict(new_data)
    
    # Based on the prediction class, determine a final bonus amount based on tasks
    if prediction_class[0] == 1:
        # A simple calculation for a bonus amount based ONLY on completed tasks
        bonus_amount = 500 + (tasks_completed * 20) + (on_time_percentage * 5)
    else:
        # If model predicts 0, still provide a bonus based on completed tasks
        bonus_amount = base_bonus + (tasks_completed * 20)
        
    return bonus_amount

class OverallBonusPredictionAPIView(APIView):
    def get(self, request, *args, **kwargs):
        company_id = request.query_params.get('company_id')
        if not company_id:
            return Response(
        ...
        employees = Employee.objects.filter(company_id=company_id)
        for emp in employees:
            user_data[f'emp_{emp.employee_id}'] = {
                'user_name': emp.name,
                'user_type': "Employee",
                'total_tasks_completed': 0,
                'total_tasks_assigned': 0,
                'is_manager': 0,
                'predicted_total_bonus': 0.0
            }
        
        managers = Manager.objects.filter(company_id=company_id)
        for mgr in managers:
            user_data[f'mgr_{mgr.manager_id}'] = {
                'user_name': mgr.name,
                'user_type': "Manager",
                'total_tasks_completed': 0,
                'total_tasks_assigned': 0,
        ...
        for assignment in task_assignments:
            user_key = None
            if assignment.employee:
                user_key = f'emp_{assignment.employee.employee_id}'
            elif assignment.manager:
                user_key = f'mgr_{assignment.manager.manager_id}'
            
            if user_key in user_data:
                user_data[user_key]['total_tasks_assigned'] += 1
                if assignment.is_completed_by_user:
                    user_data[user_key]['total_tasks_completed'] += 1

        predictions_list = []
        for key, data in user_data.items():
            predicted_bonus = predict_overall_bonus(
                tasks_completed=data['total_tasks_completed'],
                is_manager=data['is_manager']
            )
            data['predicted_total_bonus'] = predicted_bonus
            predictions_list.append(data)

        serializer = OverallBonusPredictionSerializer(predictions_list, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
# your_project/your_app/views.py
...
# Load the trained ML model once when the Django app starts
ml_model = None
try:
    MODEL_PATH = os.path.join(settings.BASE_DIR, 'your_app_name', 'task_prediction_model.pkl')
    ml_model = joblib.load(MODEL_PATH)
except FileNotFoundError:
    print("WARNING: ML model file not found. Please train the model and ensure the path is correct.")
except Exception as e:
...
# This is a placeholder prediction function. You should adapt it to your model's needs.
def predict_task_status(task):
    if ml_model is None:
    ...
    features = pd.DataFrame([
        ...
        {
            'days_until_due': (task.end_date - timezone.now().date()).days,
            'is_completed': task.is_completed,
            # Add other features like 'assigned_to_manager', etc., if needed
        }
    ])

    # Get the prediction from the model
    prediction_result = ml_model.predict(features)[0]
...
class TaskWithPredictionsAPIView(APIView):
    def get(self, request, *args, **kwargs):
        company_id = request.query_params.get('company_id')
        if not company_id:
            return Response(
        ...
        # CORRECTED: Filter tasks by company_id
        ...
        for task in tasks:
            ...
            if assignment:
                if assignment.employee:
                    assigned_to_name = assignment.employee.name
                elif assignment.manager:
                    assigned_to_name = assignment.manager.name

            # Predict the status
            prediction = predict_task_status(task)

            # Manually build the data dictionary for the response
            task_data = {
                'id': task.id,
                'title': task.title,
                'description': task.description,
            ...
            tasks_with_predictions.append(task_data)

        return Response(tasks_with_predictions, status=status.HTTP_200_OK)
...
from rest_framework.decorators import api_view
from rest_framework.response import Response
from .models import Attendance, Employee, Manager

@api_view(['GET'])
def monthly_metrics(request):
    """
    Returns summary metrics for the current month.
    """
    company_id = request.query_params.get('company_id')
    if not company_id:
    ...
    total_hours = Attendance.objects.filter(
        ...
        date__lte=today,
        company_id=company_id
    ).aggregate(total_h=Sum(F('real_exit_time') - F('real_entry_time')))['total_h'] or 0

    overtime_hours = 0 # You need to calculate this based on your logic

    # Calculate on-time rate and employee present
    ...
    on_time_checkins = Attendance.objects.filter(
        date__gte=start_of_month,
        date__lte=today,
        company_id=company_id,
        real_entry_time__lte=F('expected_entry_time')
    ).count()

    on_time_rate = on_time_checkins / total_checkins if total_checkins > 0 else 0

    present_today = Attendance.objects.filter(
        date=today,
        company_id=company_id
    ).distinct('employee', 'manager').count()
    ...
    return Response({
        ...
        "employees_present": present_today,
...
# You will need to install a PDF library like ReportLab or xhtml2pdf
# attendance_app/views.py
...
def monthly_metrics(request):
    """
    Returns summary metrics for the current month for the top cards.
    Calculations are done in Python to support SQLite.
    """
    company_id = request.query_params.get('company_id')
    if not company_id:
        return Response({"error": "Company ID is required."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        total_employees_and_managers = Employee.objects.filter(company_id=company_id).count() + Manager.objects.filter(company_id=company_id).count()

        monthly_records = Attendance.objects.filter(
            date__gte=start_of_month,
            date__lte=today,
            company_id=company_id
        ).exclude(Q(real_entry_time__isnull=True) & Q(real_exit_time__isnull=True))

        total_hours_sum = 0
        total_overtime_sum = 0
        on_time_count = 0
        total_checkins = monthly_records.exclude(real_entry_time__isnull=True).count()

        for record in monthly_records:
            if record.real_entry_time and record.real_exit_time:
                entry_time = record.real_entry_time
                exit_time = record.real_exit_time
                delta = timedelta(
                    hours=exit_time.hour - entry_time.hour,
                    minutes=exit_time.minute - entry_time.minute,
                    seconds=exit_time.second - entry_time.second
                )
                total_hours_sum += delta.total_seconds() / 3600

                if record.expected_entry_time and record.expected_exit_time:
                    expected_shift_duration = timedelta(
                        hours=record.expected_exit_time.hour - record.expected_entry_time.hour,
                        minutes=record.expected_exit_time.minute - record.expected_entry_time.minute
                    ).total_seconds() / 3600
                    
                    if delta.total_seconds() / 3600 > expected_shift_duration:
                        total_overtime_sum += (delta.total_seconds() / 3600) - expected_shift_duration
            
            if record.real_entry_time and record.expected_entry_time:
                if record.real_entry_time <= record.expected_entry_time:
                    on_time_count += 1
        
        on_time_rate = (on_time_count / total_checkins) if total_checkins > 0 else 0
        
        return Response({
            "total_hours": f"{total_hours_sum:.1f}",
            "on_time_rate": f"{on_time_rate:.2f}",
            "employees_present": present_today_count,
            "total_employees": total_employees_and_managers,
            "overtime_hours": f"{total_overtime_sum:.1f}"
        })
...
def monthly_attendance_summary(request):
    """
    Returns a detailed monthly attendance summary for all users in a company.
    Calculations are done in Python to support SQLite.
    """
    company_id = request.query_params.get('company_id')
    if not company_id:
    ...
    try:
        ...
        for emp in employees:
            monthly_attendance = Attendance.objects.filter(
                ...
                date__gte=start_of_month,
                date__lte=today
            ).exclude(real_entry_time__isnull=True)

            days_present = monthly_attendance.count()
            ...
            total_entry_seconds, total_exit_seconds = 0, 0
            valid_entries, valid_exits = 0, 0
            for record in monthly_attendance:
                if record.real_entry_time:
                    total_entry_seconds += (record.real_entry_time.hour * 3600 + record.real_entry_time.minute * 60)
                    valid_entries += 1
                if record.real_exit_time:
                    total_exit_seconds += (record.real_exit_time.hour * 3600 + record.real_exit_time.minute * 60)
                    valid_exits += 1

            avg_entry_str = 'N/A'
            if valid_entries > 0:
                avg_seconds = int(total_entry_seconds / valid_entries)
                avg_entry_str = time(hour=avg_seconds // 3600, minute=(avg_seconds % 3600) // 60).strftime('%H:%M')

            avg_exit_str = 'N/A'
            if valid_exits > 0:
                avg_seconds = int(total_exit_seconds / valid_exits)
                avg_exit_str = time(hour=avg_seconds // 3600, minute=(avg_seconds % 3600) // 60).strftime('%H:%M')
            ...
            users_summary.append({
                ...
                "days_absent": days_absent,
                "late": late_count,
                "avg_entry": avg_entry_str,
                "avg_exit": avg_exit_str,
            })

        managers = Manager.objects.filter(company_id=company_id)
        for mgr in managers:
            # You can add logic for managers here if needed
            pass

        return Response(users_summary, status=status.HTTP_200_OK)
...
def export_monthly_report(request):
    """
    Generates a monthly attendance report in PDF format.
    """
    company_id = request.query_params.get('company_id')
    if not company_id:
        return Response({"error": "Company ID is required."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="Monthly_Attendance_Report_Company_{company_id}.pdf"'
        ...
        title_style = ParagraphStyle('Title', parent=styles['Normal'], fontSize=20, alignment=1, spaceAfter=12)
        Story.append(Paragraph(f"Monthly Attendance Report for Company ID: {company_id}", title_style))
        Story.append(Spacer(1, 12))

        report_data = monthly_attendance_summary(request).data
        if not report_data:
            Story.append(Paragraph("No attendance data found for this month.", styles['Normal']))
        else:
            table_data = [['Name', 'Role', 'Days Present', 'Days Absent', 'Late', 'Avg. Entry', 'Avg. Exit']]
            for item in report_data:
                table_data.append([
                    item['name'], item['role'], item['days_present'], item['days_absent'], item['late'], item['avg_entry'], item['avg_exit']
                ])
                
            table_style = TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#259696')),
...
@api_view(['GET'])
def monthly_daily_ratio(request):
    """
    Returns daily attendance ratio for the last 30 days.
    """
    company_id = request.query_params.get('company_id')
    if not company_id:
        return Response({"error": "Company ID is required."}, status=400)
    ...
    total_users = Employee.objects.filter(company_id=company_id).count() + Manager.objects.filter(company_id=company_id).count()
    ...
    while current_date <= today:
        ...
        attendance_ratio = (present_count / total_users) if total_users > 0 else 0
...
@api_view(['GET'])
def monthly_punctuality_ratio(request):
    """
    Returns daily punctuality ratio for employees and managers for the last 30 days.
    """
    company_id = request.query_params.get('company_id')
    if not company_id:
        return Response({"error": "Company ID is required."}, status=400)
    ...
    current_date = start_date
    while current_date <= today:
        daily_employees = Employee.objects.filter(company_id=company_id)
        daily_managers = Manager.objects.filter(company_id=company_id)

        # Employee Punctuality
        total_employees = daily_employees.count()
        employee_on_time = Attendance.objects.filter(
            date=current_date,
            company_id=company_id,
            employee__in=daily_employees,
            real_entry_time__lte=F('expected_entry_time')
        ).count()
        employee_ratio = (employee_on_time / total_employees) if total_employees > 0 else 0

        # Manager Punctuality
        total_managers = daily_managers.count()
        manager_on_time = Attendance.objects.filter(
            date=current_date,
            company_id=company_id,
            manager__in=daily_managers,
            real_entry_time__lte=F('expected_entry_time')
        ).count()
        manager_ratio = (manager_on_time / total_managers) if total_managers > 0 else 0
    ...
    return Response(punctuality_data)